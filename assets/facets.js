class FacetFiltersForm extends HTMLElement {
  ...
  static renderPage(searchParams, event, updateURLHash = true) {
    ...
    if(document.getElementById('ProductGridContainer').querySelector('.collection')) document.getElementById('ProductGridContainer').querySelector('.collection').classList.add('loading');
    if (countContainer){
    ...
    input.addEventListener("input", e =>{
        ...
        if((maxPrice - minPrice >= priceGap) && maxPrice <= rangeInput[1].max){
            if(e.target.className === "range-min"){
                ...
                range.style.right = (100 - (maxPrice / rangeInput[1].max) * 100) - 5 + "%";
...
rangeInput.forEach(input => {
  ...
  if(priceInput[0].value === '' || priceInput[1].value === '') {
    console.log(((minVal / rangeInput[0].max) * 100), 100 - (maxVal / rangeInput[1].max) * 100)
    range.style.left = 0;
    range.style.right = 0;
